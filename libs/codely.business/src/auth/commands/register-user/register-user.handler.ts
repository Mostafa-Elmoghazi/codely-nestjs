  ...
  implements ICommandHandler<RegisterUserCommand>
{
  ...
  async execute(command: RegisterUserCommand): Promise<User | null> {
    const clonedPayload = {
      ...
      statusId: StatusEnum.notVerified,
    };
    ...
    const user = await this.userRepository.create(clonedPayload as any);
    ...
    const hash = await this.jwtService.signAsync(
      ...
      },
      {
        ...
        expiresIn:
          this.configService.auth().AUTH_CONFIRM_EMAIL_TOKEN_EXPIRES_IN,
      },
    );
